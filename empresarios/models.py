...
# mark_safe permite enviar código html de forma segura para a view, caso contrário iria exibir
# como texto comum. É proteção contra inserção de código malicioso

    @property
    def status(self):
        if date.today() > self.data_final_captacao:
            return mark_safe('<span class="badge text-bg-success">Captação concluída</span>')
