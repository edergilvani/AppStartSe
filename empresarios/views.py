...
def empresa(request, id):
    ...
    if empresa.user != request.user:
        messages.add_message(request, constants.ERROR, 'Essa não é sua empresa')
    ...
    if request.method == "GET":
        ...
        #ou percentual_vendido = sum(proposta_investimentos.filter(status='PA').values_list('percentual', flat=True))
        #ou total_captado = sum(proposta_investimentos.filter(status='PA').values_list('valor', flat=True))
